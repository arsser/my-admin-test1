 ...
 *
 * 使用方式：在 apps/supabase 下执行 pnpm run init-storage
 * 配置：复制 .env.example 为 .env 并填写 SUPABASE_URL、SUPABASE_SERVICE_ROLE_KEY（不复制则使用默认本地值）
 */
...
// 脚本所在包根目录（apps/supabase）
const PROJECT_ROOT = path.resolve(import.meta.dirname, '..');
const ENV_PATH = path.join(PROJECT_ROOT, '.env');
const SAMPLE_IMAGES_DIR = path.join(PROJECT_ROOT, 'seed-data', 'sample-images');
